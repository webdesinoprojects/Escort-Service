...
export default async function Home() {
  const [heroData, catsData, citiesData] = await Promise.all([
    getHeroSettings(),
    getCategories(),
    getCities(),
  ]);
  ...
  const hero = {
    hero_title: heroData?.hero_title || "Flying Solo? No Worries\nEscort is made for all.",
    hero_subtitle: heroData?.hero_subtitle || "Search or Post Your Adult Advertisement",
    hero_image_url: heroData?.hero_image_url || "",
  };

  const categories: Category[] = (catsData && catsData.length > 0) ? catsData : DEFAULT_CATEGORIES;
  const cityList: City[] = citiesData || [];

  const activeCityPills = cityList.length > 0
    ? cityList.slice(0, 3).map((c) => c.name)
    : DEFAULT_CITIES.slice(0, 3);
  const bottomCitiesList = cityList.length > 0
    ? cityList.slice(0, 5).map((c) => c.name)
    : DEFAULT_CITIES;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white font-sans text-gray-800">
      ...
      {/* ── HERO ── */}
      <section className="relative w-full h-[520px] overflow-hidden bg-[#cf4f41]">
        {hero.hero_image_url && (
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {categories.map((cat) => (
            ...
            >
              <Link
                href={`/${cat.slug}`}
                className="relative aspect-[4/3] w-full cursor-pointer group/img overflow-hidden block"
              >
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="space-y-3">
                  {activeCityPills.map((city) => (
                    <Link
                      key={city}
                      href={`/${toSlug(cat.title)}/${toSlug(city)}`}
            ...
            {categories.map((cat, idx) => (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-center gap-3.5 mt-6">
                  ...
                  {idx === 0 && (
                    <Link
                      href={`/${toSlug(cat.title)}/all-cities`}
                      className="rounded-full border border-[#cf4f41] px-6 py-2.5 text-sm sm:text-base font-semibold text-[#cf4f41] hover:bg-[#cf4f41]/5 transition-all duration-200 hover:scale-105 active:scale-95 cursor-pointer bg-white"
                    >
                      All cities
                    </Link>
